    ...
    <Card className="mb-8 bg-slate-900/40 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] rounded-2xl overflow-hidden transition-all duration-300">
      ...
      <div className={`p-6 ${editingComision ? 'bg-gradient-to-r from-blue-600/20 to-indigo-700/20 border-b border-blue-500/30' : 'bg-gradient-to-r from-amber-600/20 to-yellow-600/20 border-b border-amber-500/30'} flex justify-between items-center`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white drop-shadow-md">{editingComision ? 'Editar Liquidación' : 'Nueva Liquidación de Comisión'}</h2>
            <p className="text-slate-300 text-sm font-medium mt-0.5">Cálculo y control de pagos a asesores.</p>
          </div>
        </div>
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
          ...
          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><User className="h-4 w-4 text-slate-500 group-focus-within:text-amber-400 transition-colors"/> Asesor</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect label="Seleccione Asesor" value={comision.id_asesor} onChange={(v) => handleSelectChange('id_asesor', v)} options={asesorOptions} loading={isLoadingAdvisors} />
            </div>
          </div>

          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><Shield className="h-4 w-4 text-slate-500 group-focus-within:text-amber-400 transition-colors"/> Póliza Origen</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect label="Seleccione Póliza" value={comision.id_poliza} onChange={(v) => handleSelectChange('id_poliza', v)} options={polizaOptions} loading={isLoadingPolicies} />
            </div>
          </div>

          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><Percent className="h-4 w-4 text-slate-500 group-focus-within:text-amber-400 transition-colors"/> Esquema</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect value={comision.tipo_comision} onChange={(v) => handleSelectChange('tipo_comision', v)} options={[{id: 'porcentaje', nombre: 'Porcentaje (%)'}, {id: 'fijo', nombre: 'Monto Fijo'}]} />
            </div>
          </div>

          <div className="space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">Valor ({comision.tipo_comision === 'porcentaje' ? '%' : '$'})</Label>
          ...
          {comision.tipo_comision === 'porcentaje' && (
            <div className="space-y-2 relative">
              <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">Monto Base (Prima)</Label>
              <div className="relative group">
          ...
          {/* PANEL LED DE TOTAL A PAGAR */}
          <div className="space-y-2">
            <Label className="text-emerald-400 font-bold text-xs uppercase tracking-wider flex items-center gap-2"><DollarSign className="h-4 w-4"/> Total a Pagar</Label>
          ...
          <div className="space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">F. Generación</Label>
            <div className="relative group">
          ...
          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><CheckCircle2 className="h-4 w-4 text-slate-500 group-focus-within:text-amber-400 transition-colors"/> Estatus</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect value={comision.estatus_pago} onChange={(v) => handleSelectChange('estatus_pago', v)} options={[{id: 'pendiente', nombre: 'Pendiente'}, {id: 'pagado', nombre: 'Pagado'}, {id: 'anulado', nombre: 'Anulado'}]} />
            </div>
          </div>

          <div className="md:col-span-2 lg:col-span-3 space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">Observaciones</Label>
            <div className="relative group">
              <Info className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-amber-400 transition-colors" />
              <Input name="observaciones" value={comision.observaciones} onChange={handleChange} placeholder="Notas internas..." className="pl-10 text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 transition-all" />
            </div>
          </div>

          <div className="md:col-span-2 lg:col-span-3 flex justify-end space-x-4 mt-4 pt-6 border-t border-white/10">
            {editingComision && <Button type="button" variant="outline" className="text-slate-300 border-white/20 bg-transparent hover:bg-white/10 hover:text-white transition-all" onClick={() => setEditingComision(null)}>Descartar Cambios</Button>}
            <Button type="submit" disabled={isSubmitting} className={`shadow-lg font-black tracking-wide border transition-all active:scale-95 ${editingComision ? 'bg-blue-600/80 hover:bg-blue-500 border-blue-500/50 text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-amber-600/80 hover:bg-amber-500 border-amber-500/50 text-white shadow-[0_0_15px_rgba(245,158,11,0.5)]'}`}>
              {isSubmitting ? 'Procesando...' : <span className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5"/> {editingComision ? 'Actualizar Comisión' : 'Generar Pago'}</span>}
            </Button>
          </div>
        </form>
      </CardContent>
