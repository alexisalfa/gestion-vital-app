...
function EmpresaAseguradoraForm({ onEmpresaAseguradoraSaved, editingEmpresaAseguradora, setEditingEmpresaAseguradora, apiBaseUrl }) {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!formData.nombre.trim()) newErrors.nombre = t('aseguradoras.errNameReq');
    if (!formData.rif.trim()) newErrors.rif = t('aseguradoras.errIdReq');
    if (formData.email_contacto && !/\S+@\S+\.\S+/.test(formData.email_contacto)) {
      newErrors.email_contacto = t('aseguradoras.errEmailFormat');
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm()) {
      toast({ title: t('aseguradoras.toastIncompleteData'), description: t('aseguradoras.toastCheckFields'), variant: "destructive" });
      return;
    }
    ...
    if (!token) {
      toast({ title: t('aseguradoras.toastAuthError'), description: t('aseguradoras.toastUnauthorized'), variant: "destructive" });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || t('aseguradoras.toastSaveError'));
      }

      toast({
        title: editingEmpresaAseguradora ? t('aseguradoras.toastUpdated') : t('aseguradoras.toastRegistered'),
        description: `Los datos de ${formData.nombre} ${t('aseguradoras.toastSuccessDesc')}`,
        variant: "success",
      });
      ...
      toast({ title: t('aseguradoras.toastSaveError'), description: error.message, variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <Card className="mb-8 bg-slate-900/40 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] rounded-2xl overflow-hidden transition-all duration-300">
      ...
      <div className={`p-6 ${editingEmpresaAseguradora ? 'bg-gradient-to-r from-amber-500/20 to-orange-600/20 border-b border-amber-500/30' : 'bg-gradient-to-r from-indigo-500/20 to-purple-600/20 border-b border-indigo-500/30'} flex justify-between items-center`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white drop-shadow-md">{editingEmpresaAseguradora ? t('aseguradoras.formTitleEdit') : t('aseguradoras.formTitleNew')}</h2>
            <p className="text-slate-300 text-sm font-medium mt-0.5">
              {editingEmpresaAseguradora ? `${t('aseguradoras.formDescEdit')} ${formData.nombre}` : t('aseguradoras.formDescNew')}
            </p>
          </div>
        </div>
        {editingEmpresaAseguradora && (
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <div className="space-y-2 relative">
            <Label htmlFor="nombre" className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('aseguradoras.companyName')} <span className="text-red-400">*</span></Label>
            <div className="relative group">
              ...
              <Input id="nombre" name="nombre" value={formData.nombre} onChange={handleChange} className={`pl-10 text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-600 transition-all ${errors.nombre ? 'border-red-500 ring-1 ring-red-500' : ''}`} placeholder={t('aseguradoras.companyNamePlaceholder')} />
            </div>
            {errors.nombre && <p className="text-xs text-red-400 font-bold">{errors.nombre}</p>}
          </div>

          <div className="space-y-2 relative">
            <Label htmlFor="rif" className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('aseguradoras.idDocument')} <span className="text-red-400">*</span></Label>
            <div className="relative group">
              <Hash className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
              <Input id="rif" name="rif" value={formData.rif} onChange={handleChange} placeholder={t('aseguradoras.idDocumentPlaceholder')} className={`pl-10 uppercase text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-600 transition-all ${errors.rif ? 'border-red-500 ring-1 ring-red-500' : ''}`} />
            </div>
            {errors.rif && <p className="text-xs text-red-400 font-bold">{errors.rif}</p>}
          </div>

          <div className="space-y-2 relative">
            <Label htmlFor="email_contacto" className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('aseguradoras.email')}</Label>
            <div className="relative group">
              <Mail className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
              <Input id="email_contacto" name="email_contacto" type="email" value={formData.email_contacto} onChange={handleChange} placeholder={t('aseguradoras.emailPlaceholder')} className={`pl-10 lowercase text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-600 transition-all ${errors.email_contacto ? 'border-red-500 ring-1 ring-red-500' : ''}`} />
            </div>
            {errors.email_contacto && <p className="text-xs text-red-400 font-bold">{errors.email_contacto}</p>}
          </div>

          <div className="space-y-2 relative">
            <Label htmlFor="telefono" className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('aseguradoras.phone')}</Label>
            <div className="relative group">
          ...
          <div className="space-y-2 md:col-span-2 relative">
            <Label htmlFor="direccion" className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('aseguradoras.address')}</Label>
            <div className="relative group">
              <MapPin className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-indigo-400 transition-colors" />
              <Input id="direccion" name="direccion" value={formData.direccion} onChange={handleChange} placeholder={t('aseguradoras.addressPlaceholder')} className="pl-10 text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 placeholder:text-slate-600 transition-all" />
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex justify-end space-x-4 mt-4 pt-6 border-t border-white/10">
            ...
            <Button type="submit" disabled={isSubmitting} className={`shadow-lg font-black tracking-wide border transition-all ${editingEmpresaAseguradora ? 'bg-amber-600/80 hover:bg-amber-500 border-amber-500/50 text-white shadow-[0_0_15px_rgba(217,119,6,0.5)]' : 'bg-indigo-600/80 hover:bg-indigo-500 border-indigo-500/50 text-white shadow-[0_0_15px_rgba(79,70,229,0.5)]'}`}>
              {isSubmitting ? t('aseguradoras.processing') : (
                <span className="flex items-center gap-2">
                  <CheckCircle2 className="h-5 w-5" />
                  {editingEmpresaAseguradora ? t('aseguradoras.saveChanges') : t('aseguradoras.registerCompany')}
                </span>
              )}
            </Button>
          </div>
