...
// INJERTO: Añadimos Eye a los iconos
import { PencilIcon, Trash2Icon, Loader2, Search, FileDown, FileText, Mail, Phone, UserCheck, Eye } from 'lucide-react';
import { useConfirmation } from './ConfirmationContext'; 
import Pagination from './Pagination'; 
// INJERTO: Importamos el Modal 360
...
function AsesorList({
  ...
  return (
    <>
                    ...
                    {asesores.map((asesor) => (
                      <tr key={asesor.id} className="hover:bg-emerald-50/40 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right space-x-1">
                          ...
                          {/* INJERTO: EL BOTÓN DEL OJITO 360 PARA ASESORES */}
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            onClick={() => setSelectedAsesor360(asesor.id)} 
                            className="text-emerald-600 hover:bg-emerald-100 rounded-full transition-all"
                            title="Ver Rendimiento Comercial"
                          >
      ...
      {selectedAsesor360 && (
        <AsesorProfile360 
          asesorId={selectedAsesor360} 
          onClose={() => setSelectedAsesor360(null)} 
        />
      )}
    </>
  );
