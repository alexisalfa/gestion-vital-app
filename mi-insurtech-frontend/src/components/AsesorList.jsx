...
function AsesorList({
  ...
  const getEmpresaNombre = (asesor) => {
    ...
    return t('asesores.independent');
  };

  const getInitials = (nombre, apellido) => `${nombre?.charAt(0) || ''}${apellido?.charAt(0) || ''}`.toUpperCase();

  const asesorCsvHeaders = useMemo(() => [
    { key: 'id', label: 'ID' }, { key: 'nombre', label: t('asesores.names') }, { key: 'apellido', label: t('asesores.surnames') },
    { key: 'cedula', label: t('asesores.idDocument') }, { key: 'telefono', label: t('asesores.phone') }, { key: 'email', label: t('asesores.email') }, { key: 'empresa_nombre', label: t('asesores.thCompany') },
  ], [t]);
  ...
  const handleExportPdf = () => { setIsExporting(true); onExportPdf(asesores.map(a => ({ ...a, empresa_nombre: getEmpresaNombre(a) })), 'fuerza_de_ventas', asesorCsvHeaders, t('asesores.exportPdfTitle')); setIsExporting(false); };
        ...
        <CardContent className="p-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4 border-b border-white/10 pb-4">
            <h3 className="text-xl font-black text-white flex items-center gap-3 drop-shadow-md">
              ...
              {t('asesores.listTitle')}
              <span className="bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs py-1 px-3 rounded-full ml-2 font-bold">{totalItems} {t('asesores.advisorsCount')}</span>
            </h3>
          ...
          <form onSubmit={(e) => { e.preventDefault(); onSearch(localSearch); }} className="mb-6 flex gap-3 relative z-20">
            <div className="relative flex-1">
              ...
              <Input 
                ...
                placeholder={t('asesores.searchPlaceholder')} 
                autoComplete="off"
            ...
            <Button type="submit" className="bg-emerald-600/80 hover:bg-emerald-500 text-white border border-emerald-500/50 shadow-[0_0_10px_rgba(16,185,129,0.3)] h-10 px-6 font-bold">{t('asesores.btnSearch')}</Button>
            <Button type="button" variant="outline" onClick={() => { setLocalSearch(''); onSearch(''); }} className="text-slate-300 border-white/20 bg-transparent hover:bg-white/10 hover:text-white h-10">{t('asesores.btnClear')}</Button>
          </form>
          ...
          {asesores.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4 border-2 border-dashed border-white/10 rounded-2xl bg-black/20">
              ...
              <h3 className="text-lg font-bold text-white">{t('asesores.emptyTitle')}</h3>
              <p className="text-sm text-slate-400 text-center mt-2">{t('asesores.emptyDesc')}</p>
            </div>
          ) : (
            <div className="overflow-x-auto rounded-xl border border-white/10 shadow-inner">
              <table className="min-w-full divide-y divide-white/10">
                <thead className="bg-slate-900/50 backdrop-blur-sm">
                  <tr>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('asesores.thAdvisorId')}</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('asesores.thContact')}</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('asesores.thCompany')}</th>
                    <th className="px-6 py-4 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('asesores.thActions')}</th>
                  </tr>
                </thead>
                  ...
                  {asesores.map((asesor) => (
                    <tr key={asesor.id} className="hover:bg-white/5 transition-colors duration-200 group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right space-x-2">
                        <Button variant="ghost" size="icon" onClick={() => setSelectedAsesor360(asesor.id)} className="text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-full transition-colors" title={t('asesores.btnViewPerformance')}>
                          <Eye className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => onEditAsesor(asesor)} className="text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-full transition-colors" title={t('asesores.btnEdit')}>
                          <PencilIcon className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => confirm({ title: t('asesores.confirmDeleteTitle'), message: `${t('asesores.confirmDeleteMsg1')}${asesor.nombre}${t('asesores.confirmDeleteMsg2')}`, onConfirm: () => onDeleteAsesor(asesor.id) })} className="text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-full transition-colors" title={t('asesores.btnDelete')}>
