...
function ReclamacionForm({ onReclamacionSaved, editingReclamacion, setEditingReclamacion, apiBaseUrl, polizas = [], clientes = [], isLoadingPolicies, isLoadingClients }) {
  ...
  const { t } = useTranslation(); // 🚀 Motor Encendido
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(t('reclamaciones.processError'));

      toast({ title: t('reclamaciones.toastImportSuccess'), description: t('reclamaciones.toastSavedSuccess'), variant: "success" });
      onReclamacionSaved();
      setReclamacion(initialReclamacionState);
    ...
    <Card className="mb-8 bg-slate-900/40 backdrop-blur-xl border border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] rounded-2xl overflow-hidden transition-all duration-300">
      ...
      <div className={`p-6 ${editingReclamacion ? 'bg-gradient-to-r from-amber-500/20 to-orange-600/20 border-b border-amber-500/30' : 'bg-gradient-to-r from-red-600/20 to-rose-700/20 border-b border-red-500/30'} flex justify-between items-center`}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-black text-white drop-shadow-md">{editingReclamacion ? t('reclamaciones.formTitleEdit') : t('reclamaciones.formTitleNew')}</h2>
            <p className="text-slate-300 text-sm font-medium mt-0.5">{t('reclamaciones.formDesc')}</p>
          </div>
        </div>
      ...
      <CardContent className="p-6">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
          ...
          <div className="md:col-span-2 lg:col-span-3 space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('reclamaciones.eventDescription')}</Label>
            <div className="relative group">
              <FileText className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-red-400 transition-colors" />
              <Input name="descripcion" value={reclamacion.descripcion} onChange={handleChange} required placeholder={t('reclamaciones.eventDescriptionPlaceholder')} className="pl-10 text-white bg-black/20 border-white/10 focus:bg-black/40 focus:border-red-400 focus:ring-1 focus:ring-red-400 placeholder:text-slate-600 transition-all" />
            </div>
          </div>

          <div className="space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('reclamaciones.incidentDate')}</Label>
            <div className="relative group">
          ...
          <div className="space-y-2 relative">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider">{t('reclamaciones.reportDate')}</Label>
            <div className="relative group">
              <Calendar className="absolute left-3 top-2.5 h-4 w-4 text-slate-500 group-focus-within:text-red-400 transition-colors" />
          ...
          <div className="space-y-2 relative">
            <Label className="text-rose-400 font-bold text-xs uppercase tracking-wider">{t('reclamaciones.claimedAmountForm')}</Label>
            <div className="relative group">
          ...
          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><User className="h-4 w-4 text-slate-500 group-focus-within:text-red-400 transition-colors"/> {t('reclamaciones.affectedClient')}</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect label={t('reclamaciones.selectClient')} value={reclamacion.cliente_id} onChange={(v) => handleSelectChange('cliente_id', v)} options={clienteOptions} loading={isLoadingClients} />
            </div>
          </div>

          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><ShieldCheck className="h-4 w-4 text-slate-500 group-focus-within:text-red-400 transition-colors"/> {t('reclamaciones.linkedPolicy')}</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect label={t('reclamaciones.selectPolicy')} value={reclamacion.poliza_id} onChange={(v) => handleSelectChange('poliza_id', v)} options={polizasDelCliente} loading={isLoadingPolicies} disabled={!reclamacion.cliente_id} />
            </div>
          </div>

          <div className="space-y-2 relative group">
            <Label className="text-slate-300 font-bold text-xs uppercase tracking-wider flex items-center gap-2 mb-2"><AlertCircle className="h-4 w-4 text-slate-500 group-focus-within:text-red-400 transition-colors"/> {t('reclamaciones.processStatus')}</Label>
            <div className={selectStylesClass}>
              <HeadlessSafeSelect value={reclamacion.estado_reclamacion} onChange={(v) => handleSelectChange('estado_reclamacion', v)} options={[{id: 'Pendiente', nombre: t('reclamaciones.filterStatusPending')}, {id: 'En Proceso', nombre: t('reclamaciones.filterStatusInProcess')}, {id: 'Pagada', nombre: t('reclamaciones.filterStatusPaid')}, {id: 'Rechazada', nombre: t('reclamaciones.filterStatusRejected')}]} />
            </div>
          </div>

          <div className="md:col-span-2 lg:col-span-3 flex justify-end space-x-4 mt-4 pt-6 border-t border-white/10">
            {editingReclamacion && <Button type="button" variant="outline" className="text-slate-300 border-white/20 bg-transparent hover:bg-white/10 hover:text-white transition-all" onClick={() => setEditingReclamacion(null)}>{t('reclamaciones.discardChanges')}</Button>}
            <Button type="submit" disabled={isSubmitting} className={`shadow-lg font-black tracking-wide border transition-all active:scale-95 ${editingReclamacion ? 'bg-amber-600/80 hover:bg-amber-500 border-amber-500/50 text-white shadow-[0_0_15px_rgba(217,119,6,0.5)]' : 'bg-red-600/80 hover:bg-red-500 border-red-500/50 text-white shadow-[0_0_15px_rgba(220,38,38,0.5)]'}`}>
              {isSubmitting ? t('reclamaciones.saving') : <span className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5"/> {editingReclamacion ? t('reclamaciones.updateClaim') : t('reclamaciones.registerClaim')}</span>}
            </Button>
          </div>
        </form>
      </CardContent>
