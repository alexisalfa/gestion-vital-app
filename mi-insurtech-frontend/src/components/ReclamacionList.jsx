...
function ReclamacionList({
  ...
  const handleQuickStatusUpdate = async (id, nuevoEstado) => {
    ...
    if (nuevoEstado === 'Pagada') {
      const input = window.prompt(t('reclamaciones.promptApprove'), "0.00");
      if (input === null) return; 
      montoAprobado = parseFloat(input) || 0;
    } else {
      if (!window.confirm(t('reclamaciones.confirmReject'))) return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast({ title: t('reclamaciones.toastStatusUpdated'), description: `${t('reclamaciones.toastStatusUpdatedDesc')} ${nuevoEstado} ${t('reclamaciones.toastStatusUpdatedDesc2')}`, variant: "success" });
  ...
  const handleExportCsv = () => {
    setIsExporting(true);
    const headers = [
      { key: 'poliza_numero', label: t('reclamaciones.thPolicyDate') }, { key: 'cliente_nombre', label: t('reclamaciones.thClientClaim') },
      { key: 'fecha_reclamacion', label: 'Fecha', type: 'date' }, { key: 'monto_reclamado', label: t('reclamaciones.thClaimedAmount') },
      { key: 'estado_reclamacion', label: t('reclamaciones.thStatus') }
    ];
        ...
        <CardContent className="p-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4 border-b border-white/10 pb-4">
            <h3 className="text-xl font-black text-white flex items-center gap-3 drop-shadow-md">
              ...
              {t('reclamaciones.listTitle')}
              <span className="bg-red-500/20 border border-red-500/30 text-red-300 text-xs py-1 px-3 rounded-full ml-2 font-bold">{totalItems} {t('reclamaciones.records')}</span>
            </h3>
          ...
          <form onSubmit={handleSearchSubmit} className="mb-6 bg-black/20 p-5 rounded-xl border border-white/10 backdrop-blur-md space-y-4 z-20 relative">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1">
                <Label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('reclamaciones.filterDesc')}</Label>
                <Input 
                  value={localSearch} 
                  onChange={e => setLocalSearch(e.target.value)} 
                  placeholder={t('reclamaciones.filterDescPlaceholder')} 
                  autoComplete="off"
              ...
              <div className="space-y-1">
                <Label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('reclamaciones.filterStatus')}</Label>
                <div className={filterSelectStylesClass}>
                  <HeadlessSafeSelect value={estadoFilter} onChange={setEstadoFilter} options={[{id: '', nombre: t('reclamaciones.filterStatusAll')}, {id: 'Pendiente', nombre: t('reclamaciones.filterStatusPending')}, {id: 'En Proceso', nombre: t('reclamaciones.filterStatusInProcess')}, {id: 'Pagada', nombre: t('reclamaciones.filterStatusPaid')}, {id: 'Rechazada', nombre: t('reclamaciones.filterStatusRejected')}]} />
                </div>
              </div>
              <div className="flex items-end gap-3">
                <Button type="submit" className="flex-1 bg-red-600/80 hover:bg-red-500 text-white border border-red-500/50 shadow-[0_0_10px_rgba(220,38,38,0.3)] h-10 px-6 font-bold"><Search className="h-4 w-4 mr-2"/> {t('reclamaciones.btnFilter')}</Button>
                <Button type="button" variant="outline" onClick={handleClearFilters} className="text-slate-300 border-white/20 bg-transparent hover:bg-white/10 hover:text-white h-10">{t('reclamaciones.btnClear')}</Button>
              </div>
            </div>
          </form>
          ...
          {reclamaciones.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4 border-2 border-dashed border-white/10 rounded-2xl bg-black/20">
              ...
              <h3 className="text-lg font-bold text-white">{t('reclamaciones.emptyTitle')}</h3>
              <p className="text-sm text-slate-400 text-center mt-2 leading-relaxed">{t('reclamaciones.emptyDesc')}</p>
            </div>
          ) : (
            <div className="overflow-x-auto rounded-xl border border-white/10 shadow-inner">
              <table className="min-w-full divide-y divide-white/10">
                <thead className="bg-slate-900/50 backdrop-blur-sm">
                  <tr>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('reclamaciones.thClientClaim')}</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('reclamaciones.thPolicyDate')}</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('reclamaciones.thClaimedAmount')}</th>
                    <th className="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('reclamaciones.thStatus')}</th>
                    <th className="px-6 py-4 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('reclamaciones.thActions')}</th>
                  </tr>
                </thead>
                  ...
                  {reclamaciones.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-white/5 transition-colors duration-200 group">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-400 font-medium flex items-center gap-1.5 mt-1">
                                <AlertTriangle className="h-3 w-3 text-red-500"/> {t('reclamaciones.claimId')} {r.id}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <p className="text-sm font-black text-red-400">{t('reclamaciones.policy')} {numeroPoliza}</p>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="inline-flex items-center bg-white/5 px-2.5 py-1.5 rounded-md border border-white/10 text-sm">
                            <span className="text-slate-400 font-bold mr-2">{t('reclamaciones.claimAmount')}</span>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`px-3 py-1.5 inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-wider rounded-full border 
                            ...
                            {isPagada ? t('reclamaciones.filterStatusPaid') : isPendiente ? t('reclamaciones.filterStatusPending') : isRechazada ? t('reclamaciones.filterStatusRejected') : isEnProceso ? t('reclamaciones.filterStatusInProcess') : r.estado_reclamacion}
                          </span>
                        </td>

                        <td className="px-6 py-4 whitespace-nowrap text-right space-x-2">
                          
                          <Button variant="ghost" size="icon" onClick={() => setSelectedClient360(r.cliente_id)} className="text-slate-400 hover:text-purple-400 hover:bg-purple-500/10 rounded-full transition-colors" title={t('reclamaciones.actionViewCRM')}>
                            <Eye className="h-4 w-4" />
                          </Button>

                          {(isPendiente || isEnProceso) && (
                            <>
                              <Button variant="ghost" size="icon" onClick={() => handleQuickStatusUpdate(r.id, 'Pagada')} className="text-slate-400 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-full transition-colors" title={t('reclamaciones.actionApprove')}>
                                <CheckCircle2 className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => handleQuickStatusUpdate(r.id, 'Rechazada')} className="text-slate-400 hover:text-orange-400 hover:bg-orange-500/10 rounded-full transition-colors" title={t('reclamaciones.actionReject')}>
                                <XCircle className="h-4 w-4" />
                              </Button>
                            </>
                          )}
                          
                          <Button variant="ghost" size="icon" onClick={() => onEditReclamacion(r)} className="text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-full transition-colors" title={t('reclamaciones.actionEdit')}><PencilIcon className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" onClick={() => confirm({ title: t('reclamaciones.confirmDeleteTitle'), onConfirm: () => onDeleteReclamacion(r.id) })} className="text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-full transition-colors" title={t('reclamaciones.actionDelete')}><Trash2Icon className="h-4 w-4" /></Button>
                        </td>
                      </tr>
                    );
                  })}
